...
async function handleRegister(req, res) {
  try {
    ...
    if (existingDevice) {
      return sendJSON(res, 200, { ok: true, username: existingDevice.username, existing: true });
    }
    ...
    sendJSON(res, 201, { ok: true, username: name });
  } catch (err) {
    console.error(err);
...
async function handleLogin(req, res) {
  try {
    ...
    if (!player.password) {
      ...
      await player.save();
      return sendJSON(res, 200, { ok: true, username: player.username, deviceId: player.deviceId });
    }
    ...
    sendJSON(res, 200, { ok: true, username: player.username, deviceId: player.deviceId });
  } catch (err) {
    console.error(err);
    sendJSON(res, 500, { error: 'Sunucu hatası' });
  }
